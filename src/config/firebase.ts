...
import { CHILD_TYPE, DATABASE_TABLE_NAME, MESSAGES, NOTIFICATION_TYPE } from '../utilities/constants';
      ...
      if (snapshot.exists()) {
        snapshot.forEach((childSnapshot) => {
          ...
          if (childData.spotifyId === spotifyId) {
            sendNotification(NOTIFICATION_TYPE.INFO, `${name} ${MESSAGES.ALREADY_IN_LIBRARY}`);
          } else {
            songsRef.push(data);
            sendNotification(NOTIFICATION_TYPE.SUCCESS, `${name} ${MESSAGES.SONG_ADDED_SUCCESS}`);
          }
        });
